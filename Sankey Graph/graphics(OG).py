...
#My class to allow for alignment
class Label(GraphicsObject):

    def __init__(self, p, text):
        GraphicsObject.__init__(self, ["justify","fill","text","align","font"])
        self.anchor = p.clone()
        self.setText(text)
        self.setFill(DEFAULT_CONFIG['outline'])
        self.setOutline = self.setFill
    ...
    def draw(self, canvas):
        #Anchor both describes the point at which the label is attached and the relative positioning of the text to that anchor
        f,s,b = self.config['font']
        tk.Label(canvas, text=self.getText(), font=("{} {} {}".format(f, s, b))).place(x=self.anchor.getX(), y=self.anchor.getY(), anchor=self.config["align"])
        #return GraphWin.create_label(canvas, text=self.text, font=('Helvetica 10 underline')).place(x=self.anchor.getX(), y=self.anchor.getY(), anchor=align)
        """
        p = self.anchor
        x,y = canvas.toScreen(p.x,p.y)
        return canvas.place(x,y,options)
        """
...
def test():
    ...
    tk.Label(win, text="New Line Text", font=('Helvetica 10 underline')).place(x=1, y=100, anchor='w')
    tk.Label(win, text="New Line Text", font=('Helvetica 10 underline')).place(x=1, y=125, anchor='center')
    """
